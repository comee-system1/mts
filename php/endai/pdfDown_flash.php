...
class pdfDownflash{
	...
	public function index(){
		...
		// Zipファイル一時保存ディレクトリ
		$zipTmpDir = D_PATH_HOME.'pdf/temp/';

		$result = $zip->open($zipTmpDir.$zipFileName, ZIPARCHIVE::CREATE | ZIPARCHIVE::OVERWRITE);

		$image_data_array = $this->db->getImage();

		// 処理制限時間を外す
		set_time_limit(0);
		foreach ($image_data_array as $filepath) {
			// 取得ファイルをZipに追加していく
			if($filepath[ 'fileUpdate_ext' ]){
				$filename = basename($filepath[ 'fileUpdate_ext' ]);
				$path = D_PATH_HOME."pdf/".$filepath[ 'fileUpdate_ext' ];
